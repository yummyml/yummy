...
pub fn find_swaps(feature_names: &Vec<String>, mut columns: Vec<String>) -> Vec<(usize, usize)> {
    let mut swaps = Vec::new();

    if feature_names.len() != columns.len() {
        panic!(
            "Wrong number of columns (required: {}, provided: {})",
            feature_names.len(),
            columns.len()
        );
    }
    ...
    if f1.iter().zip(&f2).filter(|&(a, b)| a == b).count() != f1.len() {
        panic!(
            "Provided columns not match model requirements {:?}",
            feature_names
        );
    }
