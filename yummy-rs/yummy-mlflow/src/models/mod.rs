...
use crate::config::{CatboostConfig, LightgbmConfig, MLConfig};
...
impl MLModelFactory {
    pub fn new(config: MLConfig) -> Box<dyn MLModel> {
        let ml_model: Box<dyn MLModel> = if let Some(ref CatboostConfig) = config.flavors.catboost {
            Box::new(CatboostModel::new(config))
        } else if let Some(ref LightgbmConfig) = config.flavors.lightgbm {
